...
export class ChatService {
  ...
  async send(
    ...
    // WeChat bridge handles its own push — skip to avoid double-delivery
    if (options.source !== 'wechat') {
      this.pushService.pushToUser(userId, out.content).catch((err) =>
        this.logger.warn(`pushToUser failed: ${(err as Error).message}`),
      );
    }
